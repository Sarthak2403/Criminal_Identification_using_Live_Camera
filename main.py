...
# training the model. It takes 500 Images of
# every new user.
	...
	if(is_number(Id) and name.isalpha()):
		...
		while(True):
			...
			if cv2.waitKey(100) & 0xFF == ord('q'):
				break
			# break if the sample number is more than 500
			elif sampleNum > 500:
				break
		...
		row = [Id, name]
		csv_file_path = r'UserDetails\\UserDetails.csv'
		# Check if the file already exists
		if not os.path.exists(csv_file_path):
			# Create the file with ID and Name columns
...
def TrackImages():
	...
	print(df.columns)
	# df = df.assign(Id=np.repeat(1, len(df)))
	# df = df.assign(Id=[2])
	# df = df.assign(Name=['Sarthak'])
	cam = cv2.VideoCapture(0)
	...
	while True:
		...
		df.get('Id',default="N.A")
		for(x, y, w, h) in faces:
			cv2.rectangle(im, (x, y), (x + w, y + h), (225, 0, 0), 2)
			Id, conf = recognizer.predict(gray[y:y + h, x:x + w])
			if(conf < 50):
				aa = df.loc[df['ID'] == Id]['Name'].values
				# aa = df.loc[(df['Id'] == Id) & (df['Name'] == Id)].values
				# aa = df.iloc[df.get('A1') == Id]['B1'].values
				# aa = df['Labels'].loc[df['Id'] == Id]['Name'].values
				# aa = df('Label').loc[df('Id') == Id]['Name'].values
